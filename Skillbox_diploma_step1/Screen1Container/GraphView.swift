...
class GraphView: UIView {
  ...
  override func draw(_ rect: CGRect) {
    ...
    let colors = [
      CGColor.init(red: 250 / 255, green: 233 / 255, blue: 222 / 255, alpha: 1),
      CGColor.init(red: 252 / 255, green: 79 / 255, blue: 8 / 255, alpha: 1)
    ]
    ...
    let cleverString = { (original: Double) -> String in
      if original > 1000 { return "\(String(format: "%.1f", original / 1000))k" } else {
    ...
    for i in 0..<cumulativeNumber.count {
      ...
      let circle = UIBezierPath(ovalIn: CGRect(origin: point, size: CGSize(
...
extension GraphView: protocolGraphView {
  ...
  func setGraphPoints(data: [GraphData]) {
    ...
    for element in data {
      internalPoints.append(element.amount)
    }
